...
%
 for i=1:NumFiles
    ...
    filedata=xlsread(filename,1,'H:J');
    
    nSpots=xlsread(filename,1,'G:G');
    ...
    for j=2:SpotNumber
        if TrackLengths(NumberTracks)~=nSpots(j) || t(j)-t(j-1)<=0 %|| abs(y(j)-y(j-1))>0.8 || abs(x(j)-x(j-1))>0.8 || abs(t(j)-t(j-1))>10 || t(j)-t(j-1)<=0
    ...
    for j=2:SpotNumber
        if TrackLengths(TrackNumber)~=nSpots(j) || t(j)-t(j-1)<=0 %|| abs(y(j)-y(j-1))>0.8 || abs(x(j)-x(j-1))>0.8 || abs(t(j)-t(j-1))>10 || t(j)-t(j-1)<=0
