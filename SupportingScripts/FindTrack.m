...
function TrackIndex=FindTrack(Tracks,index,subset)
...
if Xrange(2)-Xrange(1)>128
    ...
    TimeFlag=1;
elseif Xrange(2)-Xrange(1)>25
    % X-axis is pixels
    if exist('Tracks','var')
        Xl=Tracks(index).matrix(:,subset,2)>=roi.Position(1)/6.25;
        Xu=Tracks(index).matrix(:,subset,2)<=(roi.Position(1)+roi.Position(3))/6.25;
    else
        error('Trajectory Structure is not correct.')
    end    
...
elseif Yrange(2)-Yrange(1)>25 && ~exist('TimeFlag','var')
    % Y-axis is pixels
    if exist('Tracks','var')
        Yl=Tracks(index).matrix(:,subset,3)>=roi.Position(2)/6.25;
        Yu=Tracks(index).matrix(:,subset,3)<=(roi.Position(2)+roi.Position(4))/6.25;
    else
        error('Trajectory Structure is not correct.')
    end
